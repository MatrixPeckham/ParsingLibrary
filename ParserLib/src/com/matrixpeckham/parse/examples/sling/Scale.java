...
public class Scale extends SlingFunction {
    ...
    /**
     * Constructs a scale with the right number of arguments, which is useful
     * for a prototypical scale.
     */
    public Scale() {
    ...
    /**
     * Constructs scale(aFrom, a, aTo, bFrom, bTo). The returned scale varies
     * from bFrom to bTo as "a" varies from aFrom to aTo. The "from" and "to"
     * functions are usually points, so that the returned scale maps between two
     * sets of coordinates.
     *
     * @param aFrom
     * @param a
    ...
    @Override
    public Scale copy() {
        return new Scale(source[0].copy(), source[1].copy(), source[2].copy(),
                source[3].copy(), source[4].copy());
    }

    /**
     * Given the source functions aFrom, a, aTo, bFrom and bTo, this function
