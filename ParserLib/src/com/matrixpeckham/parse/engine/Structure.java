...
public class Structure implements Term {
     ...
     * ConsultingStructure</code> when it participates in building a dynamic
     * rule.
     * <p>
    ...
    public int hashCode() {
        int hash = 7;
        hash = 79 * hash + Objects.hashCode(this.functor);
        hash = 79 * hash + Arrays.deepHashCode(this.terms);
        return hash;
    }
    ...
    protected static Term[] headAndTail(
        ...
        if (terms.length == 1) {
            ...
            arraycopy(terms, 1, rest, 0, rest.length);
    ...
    public String toString() {
        ...
        StringBuilder buf = new StringBuilder(functor.toString());
        if (terms.length > 0) {
            buf.append("(");
            for (int i = 0; i < terms.length; i++) {
                if (i > 0) {
